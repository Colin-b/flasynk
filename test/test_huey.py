...
def app():
    ...
    ns = flasynk.AsyncNamespaceProxy(
        api.namespace("Test space", path="/foo", description="Test"), huey_application
    )
...
def test_exception_raised_and_propagated(client):
    ...
    assert result.json == {"message": "Exception"}
...
def test_custom_unhandled_exception_raised_and_propagated(client):
    response = client.get("/foo/custom_unhandled_exception")
    status_url = assert_202_regex(response, ".*")
    status_reply = client.get(status_url)
    result_url = assert_303_regex(status_reply, ".*")
    result = client.get(result_url)
